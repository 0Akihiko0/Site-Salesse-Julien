function addNavbar(activePage = "") {
  ...
  <!-- MENU BURGER -->
  <div id="burgerMenu" class="fixed top-0 left-0 w-full h-screen bg-[#0e1117]
                              z-50 transform -translate-x-full transition-transform">
    <button id="closeBurger" class="absolute top-6 right-6 text-white text-3xl">&times;</button>

    <div class="flex flex-col justify-center items-center h-full space-y-6 px-6">
...
function navLink(href, label, page, activePage) {
  const active = page === activePage
    ...
    : "hover:text-[#00ffff]";
  return `
    <a href="${href}" class="hidden lg:inline ${active} transition-colors duration-300">
      ${label}
    </a>
  `;
}
...
function burgerLink(href, label) {
  return `
    <a href="${href}" class="text-[#00ffff] border border-[#00ffff]
